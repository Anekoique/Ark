...
pub fn unload(opts: UnloadOptions) -> Result<UnloadSummary> {
    ...
    //    setting is honored.
    // Stage A skip: worktree dirs, the per-checkout state file
    // `.ark/.state.toml`, and its co-located lock file. Active task state is
    // per-checkout and re-derivable on load via two-way reconcile against
    // captured `.ark/tasks/<*>/`.
...
fn capture_orphan_hook_entries(layout: &Layout, snapshot: &mut Snapshot) -> Result<()> {
    // Skip set must agree with Stage A above: worktrees, `.state.toml`,
    // and its lock file. The tmp-orphan filter mirrors Stage A as well.
    let cfg = WorktreeConfig::load_or_default(layout)?;
    let skip = capture_skip_paths(layout, &cfg);
    for owned in layout.owned_dirs() {
        for path in walk_files_excluding(&owned, &skip)? {
