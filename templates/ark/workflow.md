...
- **Calls:**
  ...
  - `ark agent task archive [--month YYYY-MM]` — hidden internal helper for one-off archive moves; defaults `--month` to the task's own `committed_at`.
...
- **`ark agent`** — hidden, **not semver-stable**, structural mutation only. Each subcommand prints a one-line summary; illegal transitions error out (e.g. `IllegalPhaseTransition`, `WrongTier`) — never bypass them with hand-edits. `task new`, `task resume`, and `task discard` accept `--slug` (required on all three); other verbs resolve via the topology cascade (worktree path → single active → session focus) — see `task-concurrency-control` SPEC G-9. `ark agent task commit` is the structural mutation invoked by `/ark:commit`; `ark agent task archive` is the per-slug helper that backs `ark archive`. `ark agent --help` lists the children.
