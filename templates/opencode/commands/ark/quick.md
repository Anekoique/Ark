...
Tell the user to run `/ark:commit -m "<message>"`. That single command flips phase to `Committed`, renders the journal entry, stages the Ark-managed files (workspace journal, workspace index, task.toml), and runs `git commit -m "<message>"`. The user's already-staged work lands in the same commit. See `/ark:commit` for the full contract (rollback on hook failure, `--no-commit` opt-out, slug-anchored SHA recovery).
