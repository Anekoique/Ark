---
description: Close out the current (or a named) Ark task. Atomically commits work + journal + task.toml + (deep) SPEC in one git commit.
---
...
promoted feature SPEC + features INDEX) in **one** git commit. Replaces the
older `/ark:archive`; bulk archive is now a manager-only operation via the
