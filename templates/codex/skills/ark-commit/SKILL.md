---
name: ark-commit
description: Close out an Ark task by atomically committing work + journal + task.toml + (deep tier) SPEC in one git commit. Use when the user says they're done with a task and wants to land it. Replaces the older ark-archive skill; bulk archive is now a manager-only `ark archive` CLI.
---

# `ark-commit`

Close an Ark task by committing the user's staged work plus the Ark-managed
artifacts (workspace journal entry, updated `task.toml`, and on deep tier the
promoted feature SPEC + features INDEX) in **one** git commit.
...
message — flagged FAIL items or open Findings need acknowledgement, and
the staged diff plus the recent `git log` style are the inputs to the
message generator.
...
If the user supplied `-m "<msg>"`, use it verbatim. Otherwise:
...
# or to skip the git commit + journal write entirely:
ark agent task commit --no-commit
```

`--no-commit` skips the git commit and the journal write but still flips
phase to `Committed` and (deep tier) extracts the SPEC. The user is
responsible for any follow-up commit + manual `ark-record`.

The CLI does:
- VERIFY gate check (deep refuses on `PENDING`; standard warns).
- Deep tier: SPEC extraction (`specs/features/<slug>/SPEC.md`) + features
  INDEX upsert.
- Render and append the journal entry to `.ark/workspace/<dev>/journal-N.md`
  with `**Start Head**` + `**Base Branch**` fields. Re-render the workspace
  index.
- Save updated `task.toml` (phase = `Committed`, `committed_at = now`).
- Stage exactly the Ark-managed files (journal, workspace index, task.toml,
  and deep-tier SPEC + features INDEX) — **not** `git add -A`.
- Run `git commit -m "<message>"`.

If `git commit` fails (pre-commit hook rejects, etc.), `task_commit` rolls
back every snapshot it took: `task.toml` restored, journal truncated to
its pre-append length, workspace index restored, deep-tier SPEC files
...
To recover this task's closing-commit SHA later, run:

```bash
git log -S '**Slug**: <slug>' --format=%H -n 1 -- .ark/workspace/<dev>/journal-N.md
```

This anchored search survives later journal writes (manual `ark-record`
or other tasks closing on the same `journal-N.md`).
...
- `CommitMessageRequired` — the CLI was invoked without `-m` and without
  `--no-commit`. The skill should have generated and confirmed the message
  before reaching this; treat as a logic bug.
...
Concurrent `ark-commit` invocations on the same checkout are unsupported
(the workspace journal's session-number assignment is best-effort). If two
shells run `ark-commit` simultaneously, serialize them.
