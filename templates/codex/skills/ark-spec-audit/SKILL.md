...
A rule may end its first line with an actuator tag `⟨@<kind>: <arg>⟩` saying how it is enforced: `tool` (lint/test command), `source-scan` (`<pattern> @ <glob>`), `test-binding` (a test id), or `judgment`. Tagging is optional — untagged means judgment-by-default, not a defect; this audit reports untagged rules so you can choose to tag them.
...
- **Untagged** — no `⟨@…⟩` tag. The rule's enforcement status is unknown; it rests on nothing.
