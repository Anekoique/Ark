...
| `ark upgrade` | Refresh embedded templates to the current CLI version; user-modified files are preserved (prompt) or overridden by `--force` / `--skip-modified` / `--create-new`. `[upgrade]` in `.ark/config.toml` declares `ejected` (never touched) and `merged` (diff3 3-way merge against a `.ark/.upgrade-base/` sidecar) paths. `--dry-run` previews the plan; every run backs up touched files + manifest to `.ark/.upgrade-backup/`, restorable via `--restore`. Re-applies `CLAUDE.md` block and `SessionStart` hook unconditionally (not hash-tracked).      |
