...
- 2026-07-15 `codeagent-cli-support`: `PLATFORMS` registry grows to 4; `ark init --codeagent` / `--no-codeagent` added. Codex's `ARK_CONTEXT_HOOK_COMMAND` redirect pattern (stdout suppressed via `>/dev/null`) is now shared by CodeAgent CLI's `CODEAGENT_CONTEXT_HOOK_COMMAND`.
