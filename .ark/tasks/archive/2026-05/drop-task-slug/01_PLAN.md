...
[**Added**]
- Phase 4.4: edit `worktree/SPEC.md` (R-001).
- Phase 4.5: edit both `templates/ark/workflow.md` and `.ark/workflow.md` lines 165 + 195 (R-007).
- Phase 5.1.1: programmatic SPEC-drift grep gate (R-009).
- C-9: `discard --slug` is REQUIRED, mirroring `resume`. Falls back to a hard CLI error if missing.
- C-10: worktree-path branch returns the slug only when `state.tasks.active.contains(&slug)` (R-002).
- C-11: `resolve_slug` accepts `&dyn Ppid` for testability (R-004).
- C-12: `AmbiguousActiveTask` message text simplified (R-005).
- V-UT-11: worktree path with task dir but slug NOT in active → falls through (R-002).
- V-UT-12: discard rejects missing slug at clap level (R-003).

[**Changed**]
- G-2 / G-4: `discard` no longer cascades. Slug is required.
- G-3 step 1: gates on `state.tasks.active.contains(&slug)`.
- G-3 signature: `resolve_slug(root: &Path, ppid: &dyn Ppid)`.
- G-5: `AmbiguousActiveTask` message updated.
- C-1: empty-component guard removed; non-UTF-8 case kept (R-008). V-UT-4 dropped.
- T-1 verdict: REVERSED — keep `--slug` mandatory on `discard`.
- Phase 4.3: workflow.md edits ARE required (was "no edits expected").

[**Removed**]
- V-UT-4 (impossible case per R-008).
- The `--force`-as-target-guard argument from T-1 (acknowledged unsafe).

[**Unresolved**]
- None blocking. R-006 accepted as documented limitation (NG-7), not a code change.

[**Response Matrix**]

| Source | ID | Decision | Resolution |
|--------|----|----------|------------|
| Review | R-001 | Accepted | Phase 4.4 added: edits `worktree/SPEC.md` G-2, G-4. |
| Review | R-002 | Accepted | C-10 added: gate worktree branch on active-set membership. V-UT-11 covers it. Call graph updated to load state before consulting worktree path. |
| Review | R-003 | Accepted | `discard` keeps `--slug` mandatory (G-2 reversal). C-9 added. T-1 verdict reversed. V-UT-12 covers clap-level rejection. |
| Review | R-004 | Accepted | `resolve_slug(root, ppid: &dyn Ppid)`. CLI dispatch constructs `RealPpid::new()` once at startup; tests pass `StubPpid`. C-11 added. |
| Review | R-005 | Accepted | Message simplified per C-12. |
| Review | R-006 | Accepted (documented) | Added to Non-goals as NG-7: symlinked/case-insensitive layouts are best-effort. No `canonicalize` call (would silently mask user mistakes). C-1 mentions the limitation. |
| Review | R-007 | Accepted | Phase 4.5 added: edit lines 165 + 195 in both workflow.md copies. |
| Review | R-008 | Accepted | C-1 reworded; V-UT-4 removed. |
| Review | R-009 | Accepted | Phase 5.1.1 grep gate. |
| Review | R-010 | Accepted | Body switches to `lookup_session_id(...).ok().flatten()`. |
...
CLI dispatch constructs `RealPpid::new()` once at the top of `TaskCommand::dispatch` / `SpecCommand::dispatch` / `WorktreeSubcommand` and passes a reference to all `resolve_slug` calls.
...
1. `crates/ark-cli/src/agent_cli.rs`:
   ...
   - Update every callsite in `TaskCommand::dispatch`, `SpecCommand::dispatch`, `WorktreeSubcommand` to drop the `Option<String>` arg and pass `&ppid`. Construct `let ppid = RealPpid::new();` once at the top of each `dispatch` function.
...
1. Add unit tests in `agent_cli.rs::tests` for `resolve_slug` using `StubPpid`:
   ...
   - `resolve_slug_uses_session_focus_when_ambiguous` — two actives, `StubPpid(42)`, pre-write the cache file at `cache_file_path(&layout, 42)` with a UUID, plant `[sessions.<UUID>] focus = "a"` in state.toml → returns `Ok("a")`.
...
4. `.ark/workflow.md` (NEW per R-007):
   ...
   - Re-grep `--slug` to confirm only `resume` and `discard` mentions remain.
5. `templates/ark/workflow.md`:
   - Same two edits, mirrored. Templates are embedded at build time → `cargo build` after editing.
...
1. Four-gate check: `cargo build --workspace && cargo test --workspace && cargo fmt --all -- --check && cargo clippy --workspace --all-targets -- -D warnings`.
   - 1.1 (NEW per R-009): SPEC drift gate. Run:
     ```
     ...
     ```
     Both `grep` invocations must exit non-zero (no matches). Negated by `!`, the whole pipeline succeeds only when both greps return empty.
