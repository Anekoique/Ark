...
Iteration 01 reworks the worktree protocol to fix the three CRITICALs from `00_REVIEW.md`. The biggest design shift: **drop standalone `task worktree create`** — `task new --worktree` is the only entry point that creates a worktree, and it scaffolds the task dir *inside* the worktree from the start (R-002 Option A). `walk_files` gains a skip-list so `unload`/`upgrade` don't recurse into `.ark/worktrees/` (R-003). `## Spec` is now self-contained — failure modes that used to live in Runtime are absorbed into a new `[**Failure Modes**]` block (R-001). Error names corrected, clap mutual-exclusion enforced, recursive worktree-from-worktree refused, copy-source missing now hard-fails.

## Log

[**Added**]

- New Spec block `[**Failure Modes**]` enumerating contracts that previously lived in Runtime sections (R-001 fix).
- Spec constraint that `walk_files` skips `worktrees_dir()`; `unload`/`upgrade::extract` pass that skip explicitly (R-003 fix).
- Spec constraint `Error::NestedWorktreeForbidden` when `task new --worktree` is invoked from inside an existing worktree (R-007 fix).
- Spec constraint `worktree.toml` is user-editable; never refreshed on upgrade (TR-1 / R-010 lock-in).
- Spec constraint `worktree list` is silent on zero rows (TR-3 lock-in).
- Spec constraint `task.toml.branch` is stored verbatim (TR-2 lock-in).
- Spec constraint cleanup prunes empty parent dirs under `worktrees_dir()` (R-009 fix).
- Spec constraint `task new --slug X` (no `--worktree`) writes nothing under `.ark/worktrees/` — added V-IT-10 to enforce.
- Spec constraint `task archive` does not modify the worktree dir or branch — added V-IT-9 to enforce.
- Spec error variant `Error::WorktreeCopySourceMissing { path }` (R-011 hard-fail).
- Spec error variant `Error::TaskExistsOnParent { slug }` for the dropped-standalone-create flow (R-008 fix).

[**Changed**]

- **Subcommand surface trimmed.** Dropped `ark agent task worktree create`. The two remaining subcommands are `worktree cleanup` and `worktree list`. Worktree creation is *only* via `task new --worktree`. (Resolves R-008 by eliminating the migration path that triggered it.)
- **Worktree-creation flow inverted (R-002).** Was: scaffold task dir on parent, then `git worktree add`. Now: `git worktree add` first off `<base_branch>`, scaffold task dir *inside the worktree*. Parent's working tree never sees the new task dir.
- **Atomic rollback semantics (R-004) restated** under the new flow: a) `git worktree add` fails → nothing to roll back; b) post-add scaffolding fails → `git worktree remove --force` + branch delete; c) `cfg.copy`/`post_create` fails → same as (b).
- **Error variants renamed (R-005)**: `Error::WorktreeAlreadyExists` → `Error::TaskAlreadyHasWorktree`; `Error::WorktreeExists` → `Error::WorktreeDirExists`. Naming now reads naturally: "task already has a worktree" vs "worktree dir already exists".
- **CLI mutual exclusion (R-006)**: `--branch-type` and `--branch` carry `conflicts_with` at clap level.
- **Copy-source missing policy (R-011)**: was "warn and skip", now "hard fail". Aligns with deep-tier intent of failing fast at boundaries.
- **`worktree.toml` key count (R-010)**: G-1 corrected from 3 keys to 4 (adds `branch_prefix`).
- **Slug validation note**: `--branch <full>` may name a branch unrelated to the task slug; this is allowed (TR-2). Added explicit Spec note.

[**Removed**]

- The `WorktreeCreate*` types and `worktree_create` standalone function. Their logic moves into `task::new`'s `--worktree` path as a private helper.
- Standalone CLI subcommand `ark agent task worktree create`.
- Goal G-7's "or via the convenience flag" wording — `task new --worktree` is now the *sole* path, not a convenience.
- The "log warning, skip" copy-missing semantic.

[**Unresolved**]

- None blocking. The two open questions Q-1 and Q-2 from 00_PLAN are now locked in (TR-1, TR-2). Q-3 is locked (TR-3).

[**Response Matrix**]

| Source | ID | Decision | Resolution |
|--------|----|----------|------------|
| Review | R-001 | Accepted | Added `[**Failure Modes**]` Spec block; absorbed copy-missing policy, BranchInUse detection mechanism, rollback ordering, archived-task gating into Spec. Removed all V-* references from inside Spec. |
| Review | R-002 | Accepted | Adopted Option A (worktree-first). `task new --worktree` runs `git worktree add` first off `<base_branch>`, then scaffolds the task dir inside the worktree. Parent untouched. New constraint C-2 documents the protocol. |
| Review | R-003 | Accepted | Added `walk_files_excluding(root, skip_under)` helper. `unload` and `upgrade::extract` pass `&[layout.worktrees_dir()]`. New constraint C-7 documents this. |
| Review | R-004 | Accepted | Restated rollback under Option A (see Failure Modes block). New V-IT covers each failure point. |
| Review | R-005 | Accepted | Renamed variants per reviewer recommendation. |
| Review | R-006 | Accepted | Added `conflicts_with = "branch"` to `WorktreeCreateCliArgs.branch_type` (and `requires = "worktree"` for both). New V-UT-10 enforces. |
| Review | R-007 | Accepted | New constraint C-13: `task new --worktree` refuses when `cwd` resolves under any `*/.ark/worktrees/*/`. New V-IT-3. |
| Review | R-008 | Accepted | Mooted by R-002 fix. Standalone `worktree create` dropped; the migration scenario no longer arises. New error `Error::TaskExistsOnParent` covers users who try the old `task new` then `worktree create` flow. |
| Review | R-009 | Accepted | Cleanup prunes empty parents up to (not including) `worktrees_dir()` itself. New constraint C-15. |
| Review | R-010 | Accepted | Spec G-1 updated: `worktree.toml` is user-editable (`ark init` writes it, `ark upgrade` does not touch it). Key count corrected to 4. New constraint C-9. |
| Review | R-011 | Accepted | Hard-fail with new error `Error::WorktreeCopySourceMissing`. New V-IT-7. |
| Review | R-012 | Accepted | Acceptance Mapping rewritten; added V-IT-9, V-IT-10, V-UT-10. |
| Review | TR-1 | Accepted | `worktree.toml` user-editable, not refreshed on upgrade. Locked in C-9. |
| Review | TR-2 | Accepted | Branch stored verbatim. Locked in C-12. |
| Review | TR-3 | Accepted | `worktree list` silent on zero rows; exit 0. Locked in C-14. |

> Rules:
> - Every prior HIGH / CRITICAL finding must appear here. ✓ R-001..R-008 all present.
> - Every Master directive must appear here. (None this iteration.)
> - Rejections must include explicit reasoning. (None — all accepted.)

[**Post-Review Revisions (applied inline; verdict was Approved with Revisions, 0 blocking)**]

| Source | ID | Decision | Resolution |
|--------|----|----------|------------|
| Review | R-101 | Accepted | Dropped `upgrade.rs` from G-10 / C-7 / Phase 1.5 / V-IT-14. `upgrade::extract` does not call `walk_files`. Constraint now focuses on `unload.rs`'s **two** `walk_files` callsites (capture loop + `capture_orphan_hook_entries`). V-IT-14 marked removed; numbering preserved. |
| Review | R-102 | Accepted | Pinned `walk_files_excluding` semantics in C-7: lexical `Path::starts_with`; `root` and `skip_under` MUST be absolute; no symlink canonicalization. Pinned wrapper question: `walk_files(root)` is preserved as a thin wrapper calling `walk_files_excluding(root, &[])`. |
| Review | R-103 | Accepted | New constraint C-21: `task.toml.worktree_path` is project-relative (forward-slash on disk). Resolved against `layout.root()` at read time. Snapshot-portable. |
| Review | R-104 | Accepted | New failure mode F-18: `base_branch lacks Ark loaded`. Includes optional probe via `git cat-file -e <base>:.ark/workflow.md` and `Error::BaseBranchLacksArk`. Probe-vs-skip decision deferred to executor. |
| Review | R-105 / TR-1 | Accepted | G-3 step 5 falls back from `git symbolic-ref --short HEAD` to `git rev-parse HEAD` on detached HEAD; SHA stored verbatim. New failure mode F-17. No new error variant. |
| Review | R-106 | Accepted | Dropped `Error::TaskAlreadyHasWorktree` from the error enum entirely. F-7 marked removed; numbering preserved. |
| Review | R-107 | Accepted | Added State Transitions note about scaffolding commit convention being user's call. |
| Review | R-108 | Accepted | G-5 + C-20 explicitly document silent-skip of worktrees whose `.current`/`task.toml` is missing/unreadable; this is the contract for non-Ark third-party worktrees under `worktrees_dir()`. |
...
[**Phase 1 — Layout, config, walk_files extension, state extensions**]

1. Add `WORKTREES_DIR` and `WORKTREE_CONFIG_FILE` constants and Layout methods to `layout.rs`.
2. Extend `TaskToml` with three optional fields. Add tests: `task_toml_loads_without_worktree_fields` (backward compat).
3. Add error variants to `error.rs` with `Display` impls.
4. Add `walk_files_excluding(root, skip_under)` to `io/fs.rs`. Tests: empty skip-list (matches `walk_files`), single skip-prefix, multiple skip-prefixes.
5. Update **both** `walk_files` callsites in `commands/unload.rs` (the `owned_dirs` capture loop and `capture_orphan_hook_entries`) to use `walk_files_excluding(_, &[layout.worktrees_dir()])`. Add regression test `unload_excludes_worktree_contents`. Per R-101: `commands/upgrade.rs` does NOT call `walk_files` and is unchanged.
6. Create `commands/agent/task/worktree/` with `mod.rs` (public types), `config.rs` (WorktreeConfig + load_or_default), `discovery.rs` (parse_git_worktree_list helper).
7. Wire re-exports in `lib.rs` and `commands/agent/mod.rs`.

Validation gate for Phase 1: `cargo test -p ark-core` passes; backward-compat test green; walk_files exclusion test green.

[**Phase 2 — `task new --worktree` end-to-end**]

1. Implement the worktree-first path in `task::new::task_new`: validate, nested-worktree check, parent-collision check, branch resolve, ref-format, base_branch, branch-in-use, dir-exists, `git worktree add`, scaffold-inside-worktree, copy, post_create.
2. Implement `cleanup_worktree(wt, branch)` private helper for rollback.
3. Wire CLI: extend `TaskNewCliArgs` with `--worktree`, `--branch-type`, `--branch` and the clap `conflicts_with` / `requires` constraints. Snapshot test: `ark agent task new --help` includes the new flags.
4. Integration tests: V-IT-1 (happy path), V-IT-2 (rollback on post_create failure), V-IT-3 (nested-worktree rejected), V-IT-7 (copy missing-source hard fail), V-IT-8 (post_create runs in worktree dir), V-IT-16, V-IT-17, V-IT-18, V-IT-19.

Validation gate for Phase 2: full `cargo test -p ark-core -p ark-cli`.

[**Phase 3 — `task worktree {cleanup,list}` + docs + .gitignore**]

1. Implement `worktree_cleanup` (discovery via `git worktree list --porcelain`, dirty check, remove, branch delete, parent pruning).
2. Implement `worktree_list` (enumerate via `git worktree list --porcelain`, filter under `worktrees_dir()`, sort).
3. Wire CLI subcommands. Snapshot test for `--help`.
4. Integration tests: V-IT-4 (cleanup happy + delete-branch), V-IT-5 (cleanup on dirty rejected), V-IT-6 (cleanup idempotence via second-run), V-IT-9 (`task archive` does not modify worktree), V-IT-10 (`task new` without `--worktree` writes nothing under `worktrees_dir()`), V-IT-11 (`worktree list` round-trip), V-IT-12 (zero-row silence), V-IT-15 (upgrade preserves worktree.toml).
5. Update `templates/ark/.gitignore` (managed block) and `templates/ark/worktree.toml` (defaults).
6. Update `templates/ark/workflow.md` and live `.ark/workflow.md` with a `### Worktree (optional)` block.
7. Manual end-to-end smoke: `ark init` fresh repo, `task new --slug demo --tier deep --worktree`, `cd` in, run `ark context`, observe correct slug, `task archive`, cleanup from parent.

Validation gate for Phase 3: `cargo test -p ark-core -p ark-cli` and the manual smoke.

---

## Trade-offs

- **T-1: Worktree storage location.** **Decision (locked from 00_PLAN):** inside the repo at `.ark/worktrees/`. R-003's recursion risk mitigated by C-7's skip-list approach.

- **T-2: `--worktree` opt-in vs auto.** **Decision (locked):** always opt-in.

- **T-3: Cleanup on archive.** **Decision (locked):** never auto-cleanup.

- **T-4: Per-task vs project-wide config.** **Decision (locked):** project-wide in `worktree.toml`.

- **T-5: Branch ref-format strategy.** **Decision (locked):** `--branch-type` curated path + `--branch <full>` escape hatch with `git check-ref-format --branch` validation.

- **T-6: `worktree.toml` management contract.** **Decision (locked, per TR-1):** user-editable; `ark init` writes from template; `ark upgrade` does not touch it.

- **T-7 (NEW): Standalone `task worktree create` vs `--worktree`-only.**
  - *Both.* Adv.: flexibility. Disadv.: cross-tree task-dir migration semantics (R-008) become a real surface to specify and test.
  - *`--worktree`-only.* Adv.: no migration story; one path to test; mooted error class. Disadv.: pre-existing tasks can't be retrofitted into a worktree (must archive + recreate).
  - **Decision: `--worktree`-only.** R-008's migration was a footgun; eliminating it removes a whole class of bugs.

- **T-8 (NEW): Where does `task.toml` live for a worktreed task?** Inside the worktree (the obvious place, given Option A). Consequence: cleanup must discover the worktree externally (C-20) since the parent has no record. Alternative (parent stub recording branch + worktree path) was considered and rejected — duplicate state diverges.
...
- **V-UT-3:** `WorktreeConfig::load_or_default` errors `InvalidConfigField` on `worktree_dir = "/abs/path"`.
- **V-UT-4:** `TaskToml` round-trips with all three new fields populated and with all three absent.
- **V-UT-5:** `task_toml_loads_without_worktree_fields` — pre-existing TOML deserializes successfully.
- **V-UT-6:** `resolve_branch` precedence: `--branch` > `--branch-type/<slug>` > `<cfg.branch_prefix>/<slug>`.
- **V-UT-7:** `--branch-type` rejects values outside `BRANCH_TYPES` with `InvalidBranchType`.
- **V-UT-8:** `Layout::worktree_dir("feat/foo")` returns `<root>/.ark/worktrees/feat/foo/`.
- **V-UT-9:** `walk_files_excluding(root, &[skip])` skips entries under `skip` and includes all others.
- **V-UT-10:** Clap parser rejects `--branch-type X --branch Y` with the standard `conflicts_with` message.
- **V-UT-11:** Clap parser rejects `--branch-type X` without `--worktree` (the `requires = "worktree"` rule).

[**Integration Tests**]

- **V-IT-1 (`task_new_worktree_happy_path`):** Scaffold a temp git repo with `ark init`. Run `task_new` with `--worktree`. Assert: worktree dir exists at `.ark/worktrees/feat/foo/`; branch `feat/foo` exists; worktree's `.ark/tasks/foo/` has PRD + task.toml; task.toml has `branch`, `worktree_path`, `base_branch`; *parent's* `.ark/tasks/foo/` does NOT exist; *parent's* `.ark/tasks/.current` unchanged; `.gitignore` block contains `.ark/worktrees/`.
- **V-IT-2 (`task_new_worktree_rollback_on_post_create`):** Configure `post_create = ["false"]`. Run `task_new --worktree`. Assert: returns `Error::PostCreateHookFailed`; worktree dir gone; branch deleted; parent unaffected.
- **V-IT-3 (`task_new_worktree_rejected_inside_worktree`):** Create one worktree, then run `task_new --worktree` with `cwd` set inside it. Assert: `Error::NestedWorktreeForbidden`.
- **V-IT-4 (`worktree_cleanup_happy_path_with_delete_branch`):** V-IT-1 + `worktree_cleanup --slug foo --delete-branch`. Assert: worktree dir gone; branch gone; empty parent dirs pruned (i.e. `.ark/worktrees/feat/` removed; `.ark/worktrees/` itself remains).
- **V-IT-5 (`worktree_cleanup_dirty_without_force_rejected`):** V-IT-1 + write a dirty file in the worktree. Run cleanup without `--force`. Assert: `Error::WorktreeDirty`; nothing removed.
- **V-IT-6 (`worktree_cleanup_idempotent_via_second_run`):** V-IT-1 + cleanup + cleanup again. Assert: second run returns `Error::WorktreeNotFound`.
- **V-IT-7 (`task_new_worktree_copy_missing_source_hard_fails`):** Configure `copy = [".env"]` with no `.env` in repo. Run `task_new --worktree`. Assert: `Error::WorktreeCopySourceMissing`; rollback happened.
- **V-IT-8 (`task_new_worktree_post_create_runs_in_worktree`):** Configure `post_create = ["touch hello.txt"]`. Run `task_new --worktree`. Assert: `<wt>/hello.txt` exists.
- **V-IT-9 (`archive_inside_worktree_leaves_worktree_intact`):** V-IT-1 + advance phases inside worktree to Verified + `task_archive`. Assert: worktree dir still exists; branch still exists; task dir moved to `<wt>/.ark/tasks/archive/YYYY-MM/foo/`.
- **V-IT-10 (`task_new_without_worktree_makes_no_worktree_changes`):** Run `task_new` without `--worktree`. Assert: `<root>/.ark/worktrees/` does not exist or is empty; behavior matches pre-feature `task_new`.
- **V-IT-11 (`worktree_list_round_trip`):** Create two worktreed tasks + one non-worktreed. Assert: `worktree_list` returns exactly two rows in `updated_at` desc order.
- **V-IT-12 (`worktree_list_zero_rows_silent`):** Run `worktree_list` on a fresh project. Assert: rows empty; `Display` writes empty string; exit 0.
- **V-IT-13 (`unload_excludes_worktree_contents`):** Create a worktree with sentinel file. Run `unload`. Assert: both `walk_files` callsites in `unload.rs` skip the worktree — `.ark.db` does NOT contain the sentinel, and `manifest.files` / `manifest.hashes` contain no path under `worktrees_dir()`. Bounded in size.
- **V-IT-14:** *(Removed per R-101 — `upgrade::extract` does not call `walk_files`, so the property is vacuously true. Slot retained to preserve numbering.)*
- **V-IT-15 (`upgrade_does_not_overwrite_worktree_toml`):** Edit `.ark/worktree.toml` post-init; run `upgrade`. Assert: file unchanged.
- **V-IT-16 (`task_new_worktree_branch_in_use_rejected`):** Pre-create branch `feat/foo` checked out in another worktree. Run `task_new --worktree --slug foo`. Assert: `Error::BranchInUse`.
- **V-IT-17 (`task_new_worktree_invalid_branch_rejected`):** Pass `--branch ".."` (or another ref-format violation). Assert: `Error::InvalidBranchName`.
- **V-IT-18 (`task_new_worktree_dir_exists_rejected`):** Pre-create `.ark/worktrees/feat/foo/` as a regular directory. Run `task_new --worktree --slug foo`. Assert: `Error::WorktreeDirExists`.
- **V-IT-19 (`task_exists_on_parent_rejected`):** Run `task_new --slug foo` (no flag), then `task_new --slug foo --worktree`. Assert: `Error::TaskExistsOnParent`.

[**Failure / Robustness Validation**]

- **V-F-1:** Rollback boundary covered by V-IT-2, V-IT-7.
- **V-F-2:** Branch already in use → V-IT-16.
- **V-F-3:** Malformed branch name → V-IT-17.
- **V-F-4:** Worktree dir exists on disk → V-IT-18.
- **V-F-5:** Corrupt config → V-UT-2.
- **V-F-6:** Absolute config path → V-UT-3.
- **V-F-7:** Dirty worktree cleanup without --force → V-IT-5.
- **V-F-8:** Cleanup on already-removed worktree → V-IT-6.
- **V-F-9:** Nested worktree from inside worktree → V-IT-3.
- **V-F-10:** Pre-existing parent task → V-IT-19.

[**Edge Case Validation**]

- **V-E-1:** Slug `worktree-support` → branch `feat/worktree-support`. No transformation.
- **V-E-2:** Two parallel worktrees on different slugs → both succeed (V-IT-11 covers).
- **V-E-3:** `task new --worktree` from a brand-new repo with no commits → both the `git symbolic-ref --short HEAD` primary and `git rev-parse HEAD` fallback fail (F-16). Acceptable; document via test `task_new_worktree_unborn_branch_errors_cleanly`.
- **V-E-4:** `task new --worktree` from a detached HEAD → `git symbolic-ref` fails, `git rev-parse HEAD` succeeds, SHA stored verbatim in `task.toml.base_branch`. Document via test `task_new_worktree_detached_head_falls_back_to_sha`.

[**Acceptance Mapping**]

| Goal / Constraint | Validation |
|-------------------|------------|
| G-1 (worktree.toml) | V-UT-1, V-UT-2, V-UT-3, V-IT-15 |
| G-2 (subcommand surface) | CLI snapshot tests in Phase 2 + Phase 3 |
| G-3 (--worktree flow) | V-IT-1, V-IT-2, V-IT-7, V-IT-8, V-IT-16, V-IT-17, V-IT-18, V-IT-19 |
| G-4 (cleanup) | V-IT-4, V-IT-5, V-IT-6 |
| G-5 (list) | V-IT-11, V-IT-12 |
| G-6 (task.toml fields) | V-UT-4, V-UT-5 |
| G-7 (opt-in) | V-IT-10 |
| G-8 (archive intact) | V-IT-9 |
| G-9 (worktree-aware context) | V-IT-1 (asserts worktree's .current), V-IT-9 (advance phases inside worktree) |
| G-10 (walk_files skip) | V-UT-9, V-IT-13 |
| G-11 (.gitignore) | V-IT-1 (asserts `.gitignore` block contains `.ark/worktrees/`) |
| G-12 (docs) | Manual review during Phase 3 |
| C-1 (config parsing) | V-UT-1, V-UT-2 |
| C-2 (worktree-first protocol) | V-IT-1 |
| C-3 (backward compat task.toml) | V-UT-5 |
| C-4 (branch types) | V-UT-7 |
| C-5 (branch validation) | V-IT-17 |
| C-6 (precedence) | V-UT-6 |
| C-7 (walk_files skip) | V-IT-13 |
| C-8 (worktree_dir relative) | V-UT-3 |
| C-9 (worktree.toml lifecycle) | V-IT-15 |
| C-10 (no bare Command::new) | Existing source-scan test extended to `task/worktree/` |
| C-11 (PathExt + Layout) | Source-scan during code review |
| C-12 (branch verbatim) | V-IT-1 (asserts task.toml.branch matches resolved name verbatim) |
| C-13 (no nested worktree) | V-IT-3 |
| C-14 (list silent on zero) | V-IT-12 |
| C-15 (parent pruning) | V-IT-4 |
| C-16 (.gitignore managed) | V-IT-1 |
| C-17 (output stability) | CLI snapshot tests |
| C-18 (archive doesn't touch worktree) | V-IT-9 |
| C-19 (no-flag baseline) | V-IT-10 |
| C-20 (cleanup discovery) | V-IT-4, V-IT-6 |
| F-1..F-16 | Covered by V-F-1..V-F-10 + V-IT failure variants above |
